...
public partial class PieceFactory : RefCounted
{
	...
	public static Piece[] CreatePieces(Color color, int backRow, int frontRow)
	{
		...
		{
			new Piece(PieceType.Rock, color, new Vector(0, backRow)),
			new Piece(PieceType.Knight, color, new Vector(1, backRow)),
			new Piece(PieceType.Bishop, color, new Vector(2, backRow)),
			new Piece(PieceType.Queen, color, new Vector(3, backRow)),
			new Piece(PieceType.King, color, new Vector(4, backRow)),
			new Piece(PieceType.Bishop, color, new Vector(5, backRow)),
			new Piece(PieceType.Knight, color, new Vector(6, backRow)),
			new Piece(PieceType.Rock, color, new Vector(7, backRow))
		};
	...
	private static List<Piece> CreatePawns(Color color, int frontRow)
	{
		...
		for (var i = 0; i < 8; i++)
		{
			result.Add(new Piece(PieceType.Pawn, color, new Vector(i, frontRow)));
		}
		return result;
	}
